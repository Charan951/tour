...
export const LeadManagementPage: React.FC = () => {
  ...
  return (
    ...
    >
      <div className="space-y-6">
        ...
        <div className="flex items-center justify-between bg-white px-5 py-3 rounded-2xl border border-slate-200/80 shadow-xs">
          <div className="flex items-center gap-2">
            ...
            <span className="text-xs font-semibold text-slate-700">Live Real-Time Auto-Sync Active</span>
            <span className="text-xs text-slate-400">({enquiries.length} total leads)</span>
          </div>
          <button
            onClick={() => fetchEnquiries()}
            className="px-3.5 py-1.5 rounded-xl bg-slate-100/80 hover:bg-ocean-600 hover:text-white text-slate-700 font-semibold text-xs transition-all flex items-center gap-1.5 cursor-pointer"
          >
        ...
        {loading ? (
          <div className="text-center py-12 text-slate-400 text-xs">Loading customer leads...</div>
        ) : enquiries.length === 0 ? (
                      ...
                      <tr key={enq._id} className="hover:bg-slate-50/80 transition-colors">
                        <td className="p-3.5">
                          <div className="flex items-center justify-between gap-2">
                            <span className="font-bold text-slate-900 text-xs sm:text-sm">{enq.fullName}</span>
                            <span className="text-[10px] font-mono text-slate-400">ID: {enq.enquiryId || enq._id.substring(0, 8)}</span>
                          </div>
                          <div className="text-[11px] font-semibold mt-1">
                            {(() => {
                              ...
                              if (pkgTitle) {
                                return <span className="text-ocean-600 font-bold">📦 {pkgTitle}</span>;
                              }
                              return <span className="text-slate-600 font-bold bg-slate-100 px-2 py-0.5 rounded-md border border-slate-200 text-[10px]">🌐 General Trip Enquiry</span>;
                            })()}
                          </div>
      ...
      {/* Create / Edit Lead Full Screen Modal */}
          ...
          <div className="sticky top-0 z-30 bg-white border-b border-slate-200/90 px-6 sm:px-10 py-4 flex items-center justify-between shadow-xs">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-xs text-slate-500">Manage customer lead contact info, preferred destination, and travel dates</p>
              </div>
            </div>
            <button
              type="button"
      ...
      {/* Lead Detail View Full Screen Modal */}
      {selectedLead && (
        <div className="fixed inset-0 z-50 bg-slate-50 overflow-y-auto flex flex-col w-full h-full min-h-screen">
          {/* Top Full-Width Header Bar */}
          <div className="sticky top-0 z-30 bg-white border-b border-slate-200/90 px-6 sm:px-10 py-4 flex items-center justify-between shadow-xs">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-xs text-slate-500">Customer travel enquiry profile and history</p>
              </div>
            </div>
            <button
              type="button"
