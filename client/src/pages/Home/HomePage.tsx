...
import { FALLBACK_PACKAGES, FALLBACK_DESTINATIONS, FALLBACK_THEMES, FALLBACK_ACTIVITIES } from '../../utils/mobileDataFallback';
...
export const HomePage: React.FC = () => {
  ...
  const fetchData = async () => {
    try {
      const [destRes, pkgRes, banRes, themeRes, actRes] = await Promise.all([
        apiClient.get('/destinations'),
        apiClient.get('/packages?limit=6'),
        apiClient.get('/banners'),
        apiClient.get('/themes'),
        apiClient.get('/activities?limit=6')
      ]);
      setDestinations(destRes.data.data || []);
      setPackages(pkgRes.data.data || []);
      setBanners(banRes.data.data || []);
      setThemeBanners(themeRes.data.data || []);
      if (actRes.data?.data && actRes.data.data.length > 0) {
              ...
              >
                <div>
                  ...
                  <div className="p-5 space-y-2">
                    <h3 className="font-['Outfit'] font-bold text-base text-slate-900 group-hover:text-ocean-600 transition-colors line-clamp-2">
                      {act.title}
                    </h3>
