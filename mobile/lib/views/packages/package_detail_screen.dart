...
class _PackageDetailScreenState extends State<PackageDetailScreen> {
  ...
  void _openBookingSheet() {
    final selectedTier = widget.package.pricingTiers.isNotEmpty && _selectedTierIndex < widget.package.pricingTiers.length
    ...
    showModalBottomSheet(
      ...
      builder: (_) => EnquiryBottomSheet(
        defaultDestination: '${widget.package.title} ${note.isNotEmpty ? "[$note]" : ""}',
      ),
    );
  }
  ...
  Widget build(BuildContext context) {
    ...
    final activeTier = tiers.isNotEmpty && _selectedTierIndex < tiers.length ? tiers[_selectedTierIndex] : null;
                ...
                children: [
                        ...
                        return Expanded(
                          child: GestureDetector(
                            ...
                            child: AnimatedContainer(
                              ...
                              margin: EdgeInsets.only(right: index == tiers.length - 1 ? 0 : 8),
                      ...
                      Container(
                        ...
                        child: Column(
                          ...
                          children: [
                            Row(
                              ...
                              children: [
                                ...
                                Container(
                                  ...
                                  child: const Text('Available', style: TextStyle(color: AppTheme.successColor, fontSize: 11, fontWeight: FontWeight.bold)),
                                ),
                              ],
                            ),
                            const SizedBox(height: 12),
                            Row(
                              children: [
                                const Icon(Icons.hotel_outlined, size: 16, color: AppTheme.primaryColor),
                                const SizedBox(width: 8),
                                Expanded(
                                  child: Text('Stay: ${activeTier.hotel}', style: const TextStyle(fontSize: 13, color: AppTheme.textPrimary, fontWeight: FontWeight.w500)),
                                ),
                              ],
                            ),
                            const SizedBox(height: 8),
                            Row(
                              children: [
                                ...
                                const SizedBox(width: 8),
                                Expanded(
                                  child: Text('Meals: ${activeTier.meal}', style: const TextStyle(fontSize: 13, color: AppTheme.textPrimary, fontWeight: FontWeight.w500)),
                                ),
                              ],
                            ),
                            const SizedBox(height: 8),
                            Row(
                              children: [
                                const Icon(Icons.directions_car_outlined, size: 16, color: Colors.teal),
                                const SizedBox(width: 8),
                                Expanded(
                                  child: Text('Transfer: ${activeTier.transport}', style: const TextStyle(fontSize: 13, color: AppTheme.textPrimary, fontWeight: FontWeight.w500)),
                                ),
                              ],
                            ),
                          ],
                      ...
                      (h) => Padding(
                        ...
                        child: Row(
                          children: [
                            ...
                            const SizedBox(width: 8),
                            Expanded(child: Text(h, style: const TextStyle(fontSize: 13, color: AppTheme.textPrimary))),
                          ],
                        ),
                      ),
                    ),
                  ...
                  if (package.itinerary.isNotEmpty) ...[
                    ...
                    ...List.generate(package.itinerary.length, (index) {
                      ...
                      final dayTitle = dayData['title'] ?? 'Sightseeing & Excursion';
                      final dayDesc = dayData['description'] ?? 'Guided local sightseeing, meals, and overnight stay at resort.';
                      ...
                      return Container(
                        ...
                        child: ExpansionTile(
                          ...
                          onExpansionChanged: (expanded) {
                            if (expanded) setState(() => _expandedDayIndex = index);
                          },
                          leading: Container(
                    ...
                    children: [
                      Expanded(
                        child: ElevatedButton(
                          onPressed: () => setState(() => _showInclusions = true),
                          style: ElevatedButton.styleFrom(
                            backgroundColor: _showInclusions ? AppTheme.successColor : Colors.grey[200],
                            foregroundColor: _showInclusions ? Colors.white : Colors.black87,
                            elevation: _showInclusions ? 2 : 0,
                            shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(10)),
                          ),
                          child: const Text('Inclusions', style: TextStyle(fontWeight: FontWeight.bold)),
                        ),
                      ),
                      const SizedBox(width: 12),
                      Expanded(
                        child: ElevatedButton(
                          onPressed: () => setState(() => _showInclusions = false),
                          style: ElevatedButton.styleFrom(
                            backgroundColor: !_showInclusions ? Colors.redAccent : Colors.grey[200],
                            foregroundColor: !_showInclusions ? Colors.white : Colors.black87,
                            elevation: !_showInclusions ? 2 : 0,
                            shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(10)),
                          ),
                          child: const Text('Exclusions', style: TextStyle(fontWeight: FontWeight.bold)),
                        ),
                      ),
                    ],
                  ),
                    ...
                    child: Column(
                      ...
                      children: _showInclusions
                          ? (package.inclusions.isNotEmpty
                              ...
                              : [const Text('Daily breakfast, hotel stay, private car transfer.')])
                              ...
                              ? package.exclusions.map((exc) => Padding(
                                  ...
                                  child: Row(
                                    children: [
                                      ...
                                      const SizedBox(width: 8),
                                      Expanded(child: Text(exc, style: const TextStyle(fontSize: 13))),
                                    ],
                                  ),
                                )).toList()
                              : [const Text('Personal shopping, airfare, entrance tickets.')]),
                    ),
                  ),
        ...
        child: Row(
          children: [
            Column(
              ...
              children: [
                Text(
                  activeTier != null ? '${activeTier.category} Plan' : 'Starting Price',
                  style: const TextStyle(color: AppTheme.textSecondary, fontSize: 11),
                ),
                Text(
                  currencyFormatter.format(activeTier != null ? activeTier.price : package.price),
